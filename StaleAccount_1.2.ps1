...
# This way when these features become standard we can change the '#requires' statement above and remove this alias.
...
$LogName = "$PSScriptRoot/{0}.log" -f ($CurrentDate.ToString("yyyyMMdd_mmss"))
...
Foreach ($StaleAccount in $StaleAccounts) {
    ...
    If (-not $ReportOnly) {
        ...
        try {
            ...
            Write-Host "Moved AD Account $($StaleAccouunt.Name) to $MoveTo"
        } catch {
            Write-Error $_.ErrorDetails
        }
