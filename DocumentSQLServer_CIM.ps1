...
#    Import-Module ImportExcel
    ...
    # Extract the domain portion of the DNS Host name to use when calling AD Commands. This is necessary for computers is Trusted Domains.
    $MachineDomainName = $DNSHostName.Substring($DNSHostName.IndexOf(".") +1, ($DNSHostName.Length - $DNSHostName.IndexOf(".")) -1)
...
Process {
    if (Test-Connection $DNSHostName -Quiet -Count 2) {
        ...
        try {
            ...
            Throw "Error accessing server $DNSHostName; $_"
        }
        ...
        foreach ($Instance in $Instances) {
            ...
            foreach ($Login in $Logins) {
                if ($Login.LoginType -eq 'WindowsUser') {
                    # Get the NetBIOS name of the domain fromthe Login.
                    Try {
                        [string]$UserDomain = $Login.Name.Split("\")[0]
                    } catch {
                        # If the domain isn't found, i.e. the domain part is 'NT Authority' use the machine domain name.
                        $UserDomain = $MachineDomainName
                    }
                    # Get the SAMAccountName part of the login name.
                    [string]$SamAccountName = $Login.Name.Split("\")[1] 
                    # If the name ends in a $ it is a computer account or a Managed Service Account.
                    If ($SamAccountName.EndsWith('$')) {
                        # check if the login is a AD computer.
                        try {
                            $User = Get-ADComputer $SamAccountName -Properties Enabled -Server $UserDomain -ErrorAction SilentlyContinue
                        } catch {
                            # Check if the login is a Managed Service Account
                            try {
                                $User = Get-ADServiceAccount $SamAccountName -Server $UserDomain -ErrorAction SilentlyContinue
                            } catch {
                                $DomainStatus = 'Not Found'
                            }
                        }
                    } else {
                        Try {
                            # Try to get the AD Object of the user.
                            $User = Get-ADUser $SamAccountName -Properties * -Server $UserDomain -ErrorAction SilentlyContinue
                        } catch {
                            # If it fails, set $User top null
                            $User = $Null
                        }
                    }
                    if ($User) {
                        If ($User.Enabled) {
                            ...
                            $DomainStatus = "N/A or not found"
                    }
                } else {
                    $DomainStatus = "N/A"
                }
                $Login | Add-Member -MemberType NoteProperty -Name "DomainStatus" -Value $DomainStatus
            }
