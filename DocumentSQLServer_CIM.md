...
# SQL Server Documentation Script

This script produces an Excel workbook that documents all aspects of an SQL server. This includes computer information, SQL Service informaiton, Databases, Logins, User Mappings, and user specific permissions.
...
- **DNCHostName**: The Server name to document. Aliases: ComputerName, Name. Accepts Pipeline input.
- **outputFolder**: The folder to place the output file in.

You can also pass an array of AD Computer Objects on the pipeline. This will produce a workbook with each SQL server in a separate worksheet.

This script uses the Common Information Model (CIM) functions in PowerShell to query information from the servers. CIM is only supported in Windows Server 2012 and later.

The script also attempts to connect to the server using SSL for CIM communications.
...
Process {
    if (Test-Connection $DNSHostName -Quiet -Count 2) {
        ...
        Write-Host "Establich CIM Session" -ForegroundColor Yellow
        try {
        ...
        foreach ($Instance in $Instances) {
            ...
            ": SQL Server Roles"
            $ServerRoles = Invoke-Sqlcmd -Query "EXEC sp_ServerRoles" -Database master -ServerInstance $InstName
            ...
            foreach ($Database in $Databases) {
                ...
                try {
                    $dbPermissions = Invoke-Sqlcmd -Query $qry_DataBasePermissions -ServerInstance $InstName
                } catch {
                    throw $_
                }
