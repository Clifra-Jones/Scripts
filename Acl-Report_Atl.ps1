...
#write rootACL
Write-ACL $rootACL
Write-AccessList $rootACL.Access

#Write the child folder acls
foreach ($childFolder in $Children) {
    $Folder = $ChildFolder.FullName
    $Index = $Children.IndexOf($ChildFolder)
    "Processing folder: {0}" -f $folder
    #Write-Progress -Activity "Getting ACL" -Status "Folder: $Folder" -PercentComplete ($index / $Children.Count * 100) -id 1
    
    if ($ComputerName) {
        $ChildACL = invoke-Command -Session $PSSession -ScriptBlock {
            Get-Acl -Path $using:folder | ConvertTo-Json
        } | ConvertFrom-Json
    } else {
        $ChildACL = Get-Acl -Path $folder
        $AccessList = $ChildACL.Access | Select-Object {$_.IsInherited -eq $false}
    }                                    
    if ($AccessList) {
        Write-ACL $ChildACL
