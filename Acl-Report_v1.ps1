...
if ($computerName) {
    if ($usePSDrive) {
        Write-Host "We are running is PSDrive Mode" -ForegroundColor Yellow
        ...
        foreach ($Child in $Children) {
            Write-Host ("Getting ACL for Folder: {0}" -f $Child.FullName) -ForegroundColor Yellow
            $Acl = Get-Acl -LiteralPath $Child.FullName | Select-Object Path, Owner, Group, Access, Audit, Sddl
        ...
        $rootACL = invoke-command -Session $PSSession -ScriptBlock { 
            Get-Acl -LiteralPath $using:Path | Select-Object Path, Owner, Group, Access, Audit, Sddl | ConvertTo-Json } | ConvertFrom-Json
        Write-Host "Getting remote folders. This may take a while..." -ForegroundColor Yellow
        ...
        foreach($Child in $Children) {
            ...
            $Acl = Invoke-Command -Session $PSSession -ScriptBlock {            
                        Get-Acl -LiteralPath $using:_.FullName | Select-Object Path, Owner, Group, Access, Audit, Sddl | ConvertTo-Json
                    } | ConvertFrom-Json
            $ChildAclList.Add($Acl)
        }
        $ChildAcls = $ChildAclList.ToArray()
...
foreach ($ACL in $Acls) {
    ...
    if ($ComputerName) {
        if ($usePSDrive) {
            # As the path in the ACL now contains a UNC path we cannot use credential to get the child items.
            # Therefor we will connect a PSDrive to the folder path to get the ACLs.
            # This PSdrive must be removed at the end of the loop so that the name won't conflict.
            if ($folder.EndsWith("\")) {
        ...
        $ChildFiles = Get-ChildItem -File -Path $folder | Sort-Object FullName
        $FileAcls = foreach ($childFile in $ChildFiles) {
            Write-Host "Getting Acl for File $($childFile.FullName)"
            Get-Acl -Path $childFile -ErrorAction SilentlyContinue
        }
    }
    foreach ($fileACL in $FileACLs) {
        ...
        Write-Host ("Getting Acl AccessList for file: {0}" -f $file)
        if ($includeInherited) {
